    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            if sample is None:
                ...
                ignore_grad = True
                sample['step']  = self.get_num_updates()
            else:
                ignore_grad = False
                sample['step']  = self.get_num_updates()
            def maybe_no_sync():
...
#            self.update_ema_variables(self.model.ema_encoder, self.model.encoder, self.model.ema_decay, self.get_num_updates() + 1)
#            self.update_ema_variables(self.model.ema_decoder, self.model.decoder, self.model.ema_decay, self.get_num_updates() + 1)
    ...
    def update_ema_variables(self, ema_model, model, alpha, step):
